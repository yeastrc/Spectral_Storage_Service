...
/**
 * Cleanup Old "Import Processing" Directories
 *
 */
...
//		log.warn("INFO:: cleanup_RemoveOldImportProcessDirectoriesUnderSuccessfulAndFailedSubdirectories(): ENTER");
		...
		try {
			{  //  Clean Successful
				...
				delete_Successful_OR_Failed( 
						...
						ConfigData_Directories_ProcessUploadInfo_InWorkDirectory.getSingletonInstance().getMax_ImportExecutionDirectoriesToKeep_SuccessfulImport(),
						ConfigData_Directories_ProcessUploadInfo_InWorkDirectory.getSingletonInstance().getMax_DaysToKeep_ImportExecutionDirectories_SuccessfulImport()
						);						
			...
			{  //  Clean Failed
				...
				delete_Successful_OR_Failed( 
						...
						ConfigData_Directories_ProcessUploadInfo_InWorkDirectory.getSingletonInstance().getMax_ImportExecutionDirectoriesToKeep_FailedImport(),
						ConfigData_Directories_ProcessUploadInfo_InWorkDirectory.getSingletonInstance().getMax_DaysToKeep_ImportExecutionDirectories_FailedImport()
						);						
	...
	private void delete_Successful_OR_Failed( 
			...
			int max_ImportExecutionDirectoriesToKeep,
			int max_DaysToKeep_ImportExecutionDirectories
		...
		long deleteDirectoryBefore_Milliseconds = System.currentTimeMillis() - ( ( (long) max_DaysToKeep_ImportExecutionDirectories ) * 1000 * 60 * 60 * 24 );
		...
		for ( int index = 0; index < scanFilesProcessedBaseDirContents.length; index++ ) {
			...
			if ( dirEntry.lastModified() < deleteDirectoryBefore_Milliseconds  
					|| index < scanFilesProcessedBaseDirContents.length - max_ImportExecutionDirectoriesToKeep ) {
